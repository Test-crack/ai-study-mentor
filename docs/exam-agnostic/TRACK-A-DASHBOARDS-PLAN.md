...
| **A2** ✅ ALREADY BUILT | Instructor **batch context switch** already exists: `BatchSelector` + `batchId` route params + server-side instructor∈batch verification on dashboard/assessment/report; management pages unscoped. Isolation correct. No backend work needed (optional: show exam name on batch pills). | batch carries its exam | A1 pattern |
| **A3** 🟡 CONTENT SCOPED (needs dev test) | Student learning content (diagnostic/drill/IA/mock questions) now scoped by `student.exam_id` — an OET student gets OET content, zero-change for IELTS. **Deferred:** the multi-exam student *picker* — blocked on the model (`InstituteStudent.user_id` is unique → one exam per student today; needs multi-enrollment schema change first). Single-exam students keep today's flow. | switch UI only appears when >1 context | A2 pattern |
| **A4** ✅ DONE (view-only) | SuperAdmin **Exam Configs** explorer: view each exam's config read-only + copy/download JSON to draft a new exam and hand it to a developer. Scoring config stays file-sourced + code-reviewed (we reverted the DB-backed edit path — a live config editor is a production-safety risk). See [A4-DB-BACKED-CONFIG.md](./A4-DB-BACKED-CONFIG.md). | config file (code-reviewed) | Phase 6 |
