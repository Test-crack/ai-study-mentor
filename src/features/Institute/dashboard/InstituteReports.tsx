  ...
  return (
    <InstituteAdminLayout activeTab="reports">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Institute-wide performance, straight from live assessment and drill data.</p>
      </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-[10px] uppercase tracking-[0.12em] text-slate-400 dark:text-slate-500">
                      <th className="py-2 pr-4 font-bold">Batch</th>
                      <th className="py-2 px-3 font-bold">Students</th>
                      <th className="py-2 px-3 font-bold">Avg Band</th>
                      <th className="py-2 px-3 font-bold">Improvement</th>
                      <th className="py-2 px-3 font-bold">IA Completion</th>
                      <th className="py-2 px-3 font-bold">Engagement</th>
                      <th className="py-2 px-3 font-bold">At Risk</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-white/[0.04]">
                    {comparison.map((b) => (
                      <tr key={b.batch_id}>
                        <td className="py-3 pr-4 text-sm font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">{b.batch_name}</td>
                        <td className="py-3 px-3 text-sm text-slate-500 dark:text-slate-400">{b.student_count}</td>
                        <td className="py-3 px-3"><BandPill band={b.avg_band} /></td>
                        <td className="py-3 px-3 text-sm font-bold">
                          {b.improvement_delta == null ? <span className="text-slate-400">—</span> : (
                            <span className={b.improvement_delta >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}>
                              {b.improvement_delta >= 0 ? "+" : ""}{b.improvement_delta.toFixed(1)}
                            </span>
                          )}
                        </td>
                        <td className="py-3 px-3 text-sm text-slate-500 dark:text-slate-400">{Math.round(b.ia_completion_rate * 100)}%</td>
                        <td className="py-3 px-3 text-sm text-slate-500 dark:text-slate-400">{Math.round(b.engagement_rate * 100)}%</td>
            ...
            {heatmap.length === 0 ? (
              ...
              <div className="space-y-4">
                {Object.entries(heatBySkill).map(([skill, rows]) => (
                  <div key={skill}>
                    <p className="text-xs font-black uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-2">{skill}</p>
                    <div className="flex flex-wrap gap-2">
                      {rows.map((r) => (
                        <div
                          key={`${r.skill}-${r.sub_skill}`}
                          className={`rounded-xl px-3 py-2 text-xs font-bold ${heatClass(r.avg_accuracy)}`}
                ...
                <p className="text-[11px] text-slate-400">Accuracy across all completed drills — hover a tile for volume.</p>
              </div>
            )}
          </SectionCard>
        </>
