...
// Initial mock data based on tutors
const initialTutors: Tutor[] = [
  { id: '1', name: 'Sarah Khan', joined: '2026-01-10', email: 'sarah.k@institute.edu', phone: '+91 9876543210', specialization: 'IELTS Preparation', status: 'ACTIVE', initials: 'SK' },
  { id: '2', name: 'Ravi Kumar', joined: '2026-01-12', email: 'ravi.k@institute.edu', phone: '+91 9876543211', specialization: 'Spoken English', status: 'ACTIVE', initials: 'RK' },
  { id: '3', name: 'Dr. Meera Joshi', joined: '2026-02-15', email: 'meera.j@institute.edu', phone: '+91 9876543212', specialization: 'Unassigned', status: 'PENDING', initials: 'MJ' },
  { id: '4', name: 'Priya Menon', joined: '2026-02-05', email: 'priya.m@institute.edu', phone: '+91 9876543213', specialization: 'IELTS Preparation', status: 'ACTIVE', initials: 'PM' },
  { id: '5', name: 'Karthik Iyer', joined: '2026-02-22', email: 'karthik.i@institute.edu', phone: '+91 9876543214', specialization: 'Unassigned', status: 'PENDING', initials: 'KI' },
];
...
export default function TutorOnboarding() {
  ...
  const handleAddTutor = (e: React.FormEvent) => {
    ...
    const newTutor: Tutor = {
      ...
      specialization: formData.specialization || 'Unassigned',
      status: 'ACTIVE',
    ...
    showToast(`Tutor "${formData.name}" onboarded successfully`);
  };

  // Handle Approve Tutor
  const handleApprove = (id: string, name: string) => {
    setTutors(tutors.map(t => t.id === id ? { ...t, status: 'ACTIVE' } : t));
    showToast(`${name} approved`);
  };

  // Helper for specialization badge styling
  const getSpecializationBadgeStyle = (specialization: string) => {
    if (specialization === 'Unassigned') {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B0A10] font-sans text-slate-900 dark:text-slate-200 transition-colors duration-300">
      ...
      <div className="hidden lg:block">
        <InstituteSidebar 
          activeTab="tutor-onboard" // Highlight appropriate tab for tutors
                      ...
                      <tr key={tutor.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="py-4 pl-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-semibold text-sm text-slate-900 dark:text-gray-200">{tutor.name}</div>
                              <div className="text-xs text-slate-500 dark:text-gray-500 mt-0.5">Joined {tutor.joined}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        {/* Contact Info */}
                        <td className="py-4">
                          <div className="text-sm font-medium text-slate-700 dark:text-gray-200">{tutor.email}</div>
                          <div className="text-xs text-slate-500 dark:text-gray-500 mt-0.5">{tutor.phone}</div>
                        </td>

                        {/* Specialization Info */}
                        <td className="py-4">
                          <span className={`px-2.5 py-1 text-[11px] font-semibold rounded border ${getSpecializationBadgeStyle(tutor.specialization)}`}>
                        ...
                        {/* Status */}
                        <td className="py-4">
                          <span className={`text-[11px] font-bold tracking-wider ${tutor.status === 'ACTIVE' ? 'text-emerald-600 dark:text-[#10B981]' : 'text-amber-600 dark:text-[#F59E0B]'}`}>
                            {tutor.status}
                          </span>
                        </td>
          ...
          <div className="fixed inset-0 bg-slate-900/50 dark:bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#15141B] border border-slate-200 dark:border-[#26252D] rounded-xl w-full max-w-[600px] overflow-hidden shadow-2xl">
              <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-[#26252D]">
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">New Tutor Onboarding</h2>
                <button 
              ...
              <form onSubmit={handleAddTutor} className="p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700 dark:text-gray-400">Specialization</label>
                    <select 
                    ...
                    >
                      ...
                      <option value="Tech Interview Prep">Tech Interview Prep</option>
