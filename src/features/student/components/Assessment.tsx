...
export default function Assessment() {
  ...
  const beginFullTest = () => {
    // Clear any leftover junk from previous tests
    localStorage.removeItem(STORAGE_KEY);
    setSkillIdx(0);
    setPhase("session");
  ...
  const handleSectionComplete = useCallback(() => {
    const res = generateMockResult(activeSkill);
    setAllResults(prev => ({ ...prev, [activeSkill]: res }));
    ...
    if (skillIdx < SKILL_ORDER.length - 1) {
      ...
      // Simulate final evaluation delay, then transition to results
      setTimeout(async () => {
        const studentId = profile?.id || "mock-student-123";
        const qualityScore = 0.85; 
        
        try {
          const apiResponse = await awardMomentum(studentId, 'FULL_ASSESSMENT_COMPLETED', qualityScore);
          setSessionMomentumAward(apiResponse.pointsAwarded);
          addPoints(apiResponse.pointsAwarded); 
        } catch (error) {
          console.error("Momentum API failed, applying fallback points.", error);
          setSessionMomentumAward(50);
          addPoints(50);
        }
  ...
  }, [activeSkill, skillIdx, profile, addPoints]);
  ...
  // Auto-advance speaking when timer hits 0
  useEffect(() => {
    if (activeSkill === "speaking" && phase === "session" && !isLoadingSession && !isRestoring && timeLeft === 0 && isRecording) {
      setIsRecording(false);
      if (sessionData && sessionData.questions[currentIdx]) {
        setRecordedPrompts(prev => ({ ...prev, [sessionData.questions[currentIdx].id]: true }));
      }
      handleNextQuestion();
    }
  }, [timeLeft, activeSkill, phase, isRecording, handleNextQuestion, sessionData, currentIdx, isLoadingSession, isRestoring]);
      ...
      <div className="min-h-[70vh] flex items-center justify-center animate-fade-in px-4 pt-12">
        <div className="max-w-lg w-full bg-white border-2 border-gray-900 rounded-2xl p-10 text-center shadow-[8px_8px_0_#0F0F0F]">
          ...
          <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tight mb-2">
            Section {skillIdx + 1} Complete
          </h2>
          <p className="text-gray-500 font-medium mb-10">
            Great job on {SKILL_LABELS[activeSkill]}. Take a deep breath.
          </p>
  ...
  const renderResults = () => {
    const scores = Object.values(allResults).map(r => r!.newBand);
    const avg = scores.reduce((a,b) => a+b, 0) / 4;
    const overallBand = Math.round(avg * 2) / 2;
    const weakestResult = Object.values(allResults).reduce((prev, curr) => (prev!.newBand < curr!.newBand ? prev : curr));
  ...
  const renderSession = () => {
    if (isLoadingSession || !sessionData) {
      return (
        <div className="min-h-[70vh] flex flex-col items-center justify-center animate-fade-in">
          ...
          <p className="text-gray-500 font-black uppercase tracking-widest text-sm">Fetching Question Set...</p>
        </div>
      );
    }
    ...
    // Validation: Check if user has answered the current question
      ...
      <div className="max-w-6xl mx-auto pt-6 pb-16 px-4 animate-fade-in">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white border-2 border-gray-900 rounded-xl p-4 sm:p-6 mb-6 gap-4" style={{ boxShadow: '6px 6px 0 #0F0F0F' }}>
          ...
          <div className="flex items-center self-end sm:self-auto bg-gray-50 border-2 border-gray-900 px-4 py-2 rounded-xl" style={{ boxShadow: 'inset 2px 2px 0 rgba(0,0,0,0.05)' }}>
            ...
            <div className="ml-3">
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Session Time</p>
          ...
          <button 
            ...
            className="hidden sm:block text-xs font-black text-gray-400 hover:text-gray-900 uppercase tracking-widest transition-colors"
          >
            Exit
          </button>
        </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4">
            {activeSkill === 'listening' ? (
              <div className="bg-indigo-50 border-2 border-gray-900 rounded-xl p-8 text-center flex flex-col items-center" style={{ boxShadow: '6px 6px 0 #0F0F0F' }}>
                ...
                <p className="text-gray-900 font-black text-xl uppercase tracking-wide mb-2">Diagnostic Audio</p>
                <p className="text-gray-600 font-medium">
                  {audioState === 'played' ? "Audio playback complete. Please finish answering the questions." : (sessionData.context || "Listen carefully. The audio will only play once.")}
                </p>
              ...
              <div className="bg-white border-2 border-gray-900 rounded-xl flex flex-col h-[700px]" style={{ boxShadow: '6px 6px 0 #0F0F0F' }}>
                <div className="p-5 border-b-2 border-gray-900 bg-gray-50 flex items-center justify-between">
                  <span className="font-black text-sm uppercase tracking-widest text-gray-500">
                    {activeSkill === 'writing' ? 'Task Context' : activeSkill === 'speaking' ? 'Speaking Instructions' : 'Reading Passage'}
                  </span>
                ...
                <div className={`p-8 overflow-y-auto ${!showPassage ? 'hidden lg:block' : 'block'}`}>
                  <pre className="font-serif whitespace-pre-wrap text-base text-gray-800 leading-loose">
                    {activeSkill === 'writing' ? (currentIdx < 5 ? sessionData.context_task1 : sessionData.context_task2) : 
                     activeSkill === 'speaking' ? (currentIdx < 5 ? sessionData.context_part12 : sessionData.context_part3) : 
                     sessionData.passage}
                  </pre>
                </div>
              </div>
