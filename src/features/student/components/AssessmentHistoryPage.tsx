      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {isMissed ? (
              <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400">Missed</span>
            ) : (
              <>
                ...
                {hasAnyAI && (
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 dark:bg-violet-500/20 dark:text-violet-400">AI Graded</span>
                )}
              </>
            )}
          </div>
...
const MockSubSkillRow = ({ score }: { score: MockSubSkillScore }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
      ...
      >
        <span className="flex-1 text-sm font-semibold text-slate-700 dark:text-slate-200">{label}</span>
        <span className="text-xs text-slate-400 shrink-0 tabular-nums">MCQ {score.correct}/{score.total_mcq}</span>
        {score.ai_band != null && (
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 overflow-hidden">
      ...
      >
        ...
        <span className="flex-1 text-sm font-semibold text-slate-700 dark:text-slate-200">{cfg?.label ?? score.skill}</span>
        <span className="text-xs text-slate-400 shrink-0 tabular-nums">{score.correct}/{score.total} correct</span>
        {score.ai_graded && (
          <span className="text-[10px] font-bold bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 px-2 py-0.5 rounded-full shrink-0">AI Graded</span>
        )}
