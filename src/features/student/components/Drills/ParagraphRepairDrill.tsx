...
export default function ParagraphRepairDrill({ prompt, onComplete }: ParagraphDrillProps) {
  ...
  // Clear answer when prompt changes
  useEffect(() => {
    setAnswer("");
  }, [prompt]);
  ...
  return (
    <div className="flex flex-col space-y-6 animate-in fade-in">
      <div className="bg-amber-50 dark:bg-amber-900/10 p-5 rounded-2xl border border-amber-200 dark:border-amber-800/50">
        <p className="text-sm font-bold text-amber-600 dark:text-amber-500 uppercase tracking-widest mb-2">Repair Task</p>
        <p className="text-slate-800 dark:text-slate-200 font-medium">
          {prompt.text} <br/><br/>
          <span className="text-slate-500">"The graph illustrates the global consumption of fast food. ___ it shows a steady increase over the last decade. ___ there was a slight dip in 2020."</span>
        </p>
      </div>
