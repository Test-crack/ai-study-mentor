    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300">
      ...
      <div className="transition-all duration-300 lg:pl-20 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-6 max-w-3xl mx-auto w-full animate-in fade-in mt-10">
          {!isComplete ? (
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-md border border-indigo-100 dark:border-slate-800">
              <div className="text-center mb-8">
                <span className="bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400 font-bold px-4 py-1.5 rounded-full text-sm tracking-wide uppercase">
                  Final Step
                </span>
                ...
                <p className="text-slate-500 mt-2">One single prompt. Show us what you absorbed from the video.</p>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      className="w-full h-48 p-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-transparent focus:border-indigo-500 outline-none resize-none transition-colors"
                      onFocus={() => { if (!isRecording) setIsRecording(true) }}
                    />
                    <div className="absolute bottom-4 right-4 text-slate-400 font-mono text-sm">
                      {formatTime(timeLeft)}
                    </div>
                  </div>
                  <div className="flex justify-end">
                    <button onClick={handleSubmit} disabled={textAnswer.length < 10} className="flex items-center px-6 py-3 bg-indigo-500 text-white font-bold rounded-xl hover:bg-indigo-600 disabled:opacity-50 transition-colors">
              ...
              {drillType === 'audio_response' && (
                <div className="flex flex-col items-center space-y-8">
                  <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl w-full text-center">
                  ...
                  {isProcessing ? (
                     <div className="flex flex-col items-center text-slate-500 space-y-3 py-10">
                       <Loader2 className="w-8 h-8 animate-spin text-indigo-500" />
                       <span className="font-medium">Finalizing session...</span>
                     </div>
                  ) : !isRecording ? (
                    <button onClick={() => setIsRecording(true)} className="relative group flex flex-col items-center justify-center w-32 h-32 rounded-full bg-indigo-500 text-white shadow-lg hover:scale-105 transition-all">
                    ...
                    <div className="flex flex-col items-center animate-in zoom-in">
                      <div className="text-4xl font-mono font-black text-indigo-500 mb-6 flex items-center gap-3">
                        <span className="w-3 h-3 rounded-full bg-red-500 animate-pulse" />
                        {formatTime(timeLeft)}
                      </div>
                      <button onClick={handleSubmit} className="flex items-center px-8 py-3 rounded-xl bg-slate-800 text-white hover:bg-slate-700 font-bold transition-all">
            ...
            // FE-14: Final Summary Completion Card
            <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-1 rounded-3xl shadow-xl animate-in zoom-in duration-500">
