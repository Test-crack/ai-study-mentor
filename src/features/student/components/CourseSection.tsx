    ...
    modules: [
      ...
      },
      {
        ...
        quiz: {
          ...
          options: ['25th of May', 'May 25', 'Twenty five May', 'The 25th'],
...
export default function CourseSection() {
  ...
  const openCourse = (course: Course) => { setSelectedCourse(course); setView('course'); };
  const openModule = (module: Module) => { setSelectedModule(module); setView('module'); };
  const startQuiz  = () => { setSelectedAnswer(null); setView('quiz'); };
  const submitQuiz = () => {
    setIsSubmitting(true);
    setTimeout(() => { setIsSubmitting(false); setView('feedback'); }, 600);
  };
  const goBack = () => {
    if (view === 'feedback' || view === 'quiz' || view === 'module') {
      setView('course'); setSelectedModule(null); setSelectedAnswer(null);
    } else if (view === 'course') {
      setView('dashboard'); setSelectedCourse(null);
    }
  };
