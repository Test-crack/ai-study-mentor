...
const StatCard = ({ value, label, color = 'text-[#3E9E93]' }: { value: React.ReactNode; label: string; color?: string; }) => (
                  ...
                  <>
                      ...
                      {cats.map(cat => {
                        ...
                        return (
                          <button key={cat} onClick={() => setActiveCategory(cat)}
                            className={`flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${activeCategory === cat ? 'bg-[#3E9E93] text-white' : 'bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 hover:bg-slate-50 border border-slate-200 dark:border-slate-800'}`}
                    ...
                    {selected && (() => {
                      ...
                      return (
                        <div className="mb-6 md:mb-8 w-full max-w-2xl bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 md:p-6 border border-slate-200 dark:border-slate-700">
                          <p className="text-[10px] md:text-xs text-[#3E9E93] dark:text-[#7FBFB6] font-semibold mb-1 md:mb-2 tracking-wider uppercase">{selected.source}</p>
                          ...
                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                            ...
                            {inCat.length > 1 && (
                              <div className="flex items-center gap-2 bg-white dark:bg-slate-900 rounded-md border border-slate-200 dark:border-slate-700 p-1 w-full sm:w-auto justify-between sm:justify-start">
                                <button onClick={() => setRepIdx(i => (i - 1 + inCat.length) % inCat.length)} className="p-2 sm:p-1 rounded text-slate-400 hover:text-[#3E9E93] transition-colors"><ChevronLeft size={16} /></button>
                                <span className="text-xs font-medium text-slate-500 min-w-[30px] text-center">{repIdx + 1}/{inCat.length}</span>
                                <button onClick={() => setRepIdx(i => (i + 1) % inCat.length)} className="p-2 sm:p-1 rounded text-slate-400 hover:text-[#3E9E93] transition-colors"><ChevronRight size={16} /></button>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 md:gap-4 items-stretch sm:items-start">
                      <button onClick={() => handleStartReading()} disabled={loadingReport || !selected}
                        className="flex items-center justify-center gap-2 bg-[#3E9E93] hover:bg-[#12897C] disabled:opacity-60 disabled:cursor-not-allowed text-white px-6 md:px-8 py-3 rounded-xl font-semibold transition-colors w-full sm:w-auto shadow-sm">
                    ...
                    {showSettings && (
                      <div className="mt-4 md:mt-6 p-4 md:p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl max-w-md shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                          ...
                          <span className="text-[#3E9E93] font-bold text-sm bg-brand-teal-50 dark:bg-[#3E9E93]/10 px-3 py-1 rounded-md">{wpm} WPM</span>
                        </div>
                        <input type="range" min="200" max="800" step="25" value={wpm} onChange={e => setWpm(Number(e.target.value))}
                          className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-[#3E9E93]" />
              ...
              <div className="flex flex-col gap-3 md:gap-4 mb-2 md:mb-4">
                <div className="flex items-center justify-between w-full">
                  ...
                  <div className="flex items-center gap-2 md:gap-3 bg-white dark:bg-slate-900 px-3 md:px-4 py-2 rounded-lg border border-slate-200 shadow-sm flex-1 md:flex-none max-w-[200px] md:max-w-none ml-2 md:ml-0">
                    <input type="range" min="200" max="800" step="25" value={wpm} onChange={e => setWpm(Number(e.target.value))}
                      className="w-full md:w-24 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-[#3E9E93]" />
                ...
                <div className="flex items-center justify-between w-full gap-2">
                  ...
                  <button onClick={() => setIsPlaying(!isPlaying)} disabled={isFinished}
                    className="flex items-center justify-center gap-1.5 md:gap-2 px-4 md:px-6 py-2 md:py-2.5 rounded-lg text-xs md:text-sm font-semibold transition-colors disabled:opacity-50 shrink-0 bg-[#3E9E93] hover:bg-[#12897C] text-white">
              ...
              <div className="flex-1 my-2 md:my-4 bg-white dark:bg-slate-900 rounded-xl md:rounded-2xl flex flex-col items-center justify-center relative overflow-hidden shadow-sm">
                {isFinished ? (
                  <div className="text-center px-4 md:px-6 space-y-4 md:space-y-6 animate-in zoom-in-95 duration-500">
                    ...
                    <p className="text-sm md:text-base text-slate-500">read at <span className="font-bold text-[#3E9E93]">{wpm} WPM</span></p>
                    {hasQuiz ? (
                      <button onClick={handleGoToQuiz} className="flex items-center justify-center gap-2 w-full md:w-auto md:mx-auto bg-[#3E9E93] hover:bg-[#12897C] text-white px-6 md:px-8 py-3 md:py-4 rounded-xl font-bold transition-colors shadow-sm mt-2 md:mt-4">
              ...
              <div className="space-y-3 md:space-y-4">
                <div className="flex items-center gap-2 md:gap-4 px-1 md:px-2">
                  ...
                  <div className="flex-1 h-1.5 md:h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                    <div className="h-full bg-[#3E9E93] transition-all duration-150 ease-linear" style={{ width: `${pct}%` }} />
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 md:gap-4">
                  <div className="bg-white dark:bg-slate-900 rounded-xl md:rounded-2xl p-2 md:p-4 shadow-sm flex flex-col items-center justify-center">
                    <div className="text-lg md:text-2xl font-bold text-[#3E9E93] mb-0.5 md:mb-1">{wpm}</div>
          ...
          {view === 'quiz' && fullReport && curQ && (
            <div className="w-full mt-2 md:mt-4 max-w-3xl mx-auto space-y-4 md:space-y-6">
              <div className="flex flex-row justify-between items-center gap-2 md:gap-4 mb-2 md:mb-4">
                ...
                <span className="inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-3 py-1 md:py-1.5 rounded-md bg-brand-teal-50 dark:bg-[#3E9E93]/10 text-[#3E9E93] text-[10px] md:text-xs font-bold uppercase tracking-widest shadow-sm">
                ...
                {qs.map((q, i) => (
                  <button key={q.id} onClick={() => setQuizIdx(i)}
                    className={`h-8 w-8 md:h-10 md:w-10 shrink-0 rounded-full flex items-center justify-center font-bold text-xs md:text-sm transition-all ${
                      i === quizIdx ? 'bg-[#3E9E93] text-white scale-110'
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl md:rounded-3xl p-5 md:p-10 shadow-sm transition-colors animate-in slide-in-from-right-4 duration-300">
                <div className="flex items-center justify-between mb-4 md:mb-6">
                  <span className={`inline-flex items-center px-2 md:px-3 py-1 rounded-md text-[10px] md:text-xs font-bold uppercase tracking-widest ${curQ.type === 'MCQ' ? 'bg-brand-teal-50 text-[#3E9E93]' : 'bg-blue-50 text-blue-600'}`}>
                    ...
                    {curQ.options.map((opt, oi) => {
                      ...
                      return (
                        <button key={oi} onClick={() => setAnswers(p => ({ ...p, [curQ.id]: opt }))}
                          className={`w-full flex items-center gap-3 md:gap-4 p-3 md:p-5 rounded-xl md:rounded-2xl border-2 text-left transition-all ${sel ? 'border-[#3E9E93] bg-brand-teal-50 dark:bg-[#3E9E93]/10' : 'border-slate-100 hover:border-slate-300 bg-white dark:bg-slate-800'}`}>
                          <span className={`w-6 h-6 md:w-8 md:h-8 rounded-md flex items-center justify-center text-xs md:text-sm font-bold shrink-0 ${sel ? 'bg-[#3E9E93] text-white' : 'bg-slate-100 text-slate-500'}`}>{letter}</span>
                          <span className={`text-sm md:text-base font-semibold ${sel ? 'text-[#3E9E93] dark:text-brand-teal-200' : 'text-slate-700 dark:text-slate-300'}`}>{opt}</span>
                        </button>
                      );
                    })}
                  </div>
              ...
              <div className="flex flex-row items-center justify-between gap-3 md:gap-4 mt-6 md:mt-8">
                ...
                {quizIdx < qs.length - 1 ? (
                  <button onClick={() => setQuizIdx(i => i + 1)}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 md:gap-2 px-4 md:px-8 py-3 rounded-xl bg-[#3E9E93] hover:bg-[#12897C] text-white font-semibold shadow-sm text-sm md:text-base">
                  ...
                  <div className="flex-1 sm:flex-none flex flex-col items-center w-full sm:w-auto">
                    <button onClick={handleSubmitQuiz} disabled={!allAnswered || submitting}
                      className="w-full flex items-center justify-center gap-1.5 md:gap-2 px-4 md:px-8 py-3 rounded-xl bg-[#3E9E93] hover:bg-[#12897C] disabled:opacity-50 text-white font-bold shadow-sm text-sm md:text-base">
          ...
          {view === 'analysis' && evaluation && fullReport && (
            <div className="w-full mt-2 md:mt-4 space-y-6 md:space-y-8 max-w-4xl mx-auto animate-in fade-in duration-500 flex flex-col items-center">
              <div className="text-center space-y-1 md:space-y-2 mb-2 w-full">
                ...
                <span className="inline-block px-2 md:px-3 py-1 bg-brand-teal-50 text-[#3E9E93] rounded-full text-[8px] md:text-[10px] font-bold uppercase tracking-widest mt-1 md:mt-2">{evaluation.speedCategory}</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 w-full">
                ...
                <StatCard value={evaluation.wpm} label="Avg WPM" color="text-[#3E9E93]" />
                <StatCard value={`${evaluation.correct}/${evaluation.total}`} label="Correct Answers" color="text-[#3E9E93]" />
              </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between sm:justify-start gap-3 mb-2">
                            ...
                            <span className={`text-[8px] md:text-[10px] px-2 py-0.5 rounded-sm font-bold uppercase tracking-widest ${a.type === 'MCQ' ? 'bg-brand-teal-50 text-[#3E9E93]' : 'bg-blue-50 text-blue-600'}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full">
                ...
                <button onClick={() => { setView('reader'); setWordIdx(0); setIsFinished(false); setIsPlaying(false); }} className="flex-1 h-12 md:h-14 rounded-xl md:rounded-2xl font-bold bg-[#3E9E93] hover:bg-[#12897C] text-white text-sm md:text-base shadow-sm">
                  Read Again
                </button>
              </div>
            </div>
