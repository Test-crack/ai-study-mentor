...
/* Tailwind Theme Maps for dynamic colors */
...
function SkillTab({
  ...
  return (
    ...
    >
      {active && (
        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${theme.glowColor}`} />
      )}
      {skill.icon}
...
function ResourceCard({
  ...
  return (
    <div
      className={`relative rounded-[20px] overflow-hidden flex flex-col transition-all duration-300 cursor-default group border
      ${isCompleted 
      ...
      <div className="relative w-full aspect-[16/9] shrink-0 overflow-hidden bg-slate-100 dark:bg-[#0a0f1e]">
        {embedId ? (
          <iframe
            ...
            allowFullScreen
            className={`block w-full h-full ${isCompleted ? 'grayscale opacity-80' : ''}`}
          />
        ) : item.thumbnail_url ? (
          <img
          ...
          <div
            className="w-full h-full flex items-center justify-center cursor-pointer bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-indigo-950/40"
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="pt-3.5 border-t border-slate-100 dark:border-white/10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={onToggle}
              title={isCompleted ? 'Mark unread' : 'Mark complete'}
        ...
        <main className="flex-1 px-6 py-8 md:p-10 lg:py-12 max-w-7xl mx-auto w-full animate-in fade-in duration-500">
            ...
            <div className="inline-flex flex-wrap items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-full p-1.5 shadow-sm gap-1">
              {SKILLS.map(s => (
                <SkillTab
                  ...
                  onClick={() => { setActiveTab(s.id); setPage(1); }}
                />
              ))}
            </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-6 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 rounded-3xl text-center shadow-sm">
              ...
              <h3 className="font-fraunces text-2xl font-bold text-slate-800 dark:text-slate-200 mb-2">
                Nothing here yet
              </h3>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all duration-300 ${
                      p === page 
                        ? 'w-7 bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]' 
                        : 'w-2 bg-indigo-200 dark:bg-indigo-500/20'
                    }`}
                  />
                ))}
              </div>
