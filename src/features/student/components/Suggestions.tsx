    ...
    >
      ...
      <div className="relative w-full aspect-[16/9] shrink-0 overflow-hidden bg-slate-100 dark:bg-[#0a0f1e]">
        {embedId ? (
          ...
          <div className="w-full h-full flex items-center justify-center cursor-pointer bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-brand-teal-950/40"
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="mb-2.5">
          <span className="text-[10px] font-bold tracking-widest uppercase text-brand-teal-600 dark:text-brand-teal-400 font-outfit">
            {item.source || 'Curated'}
          </span>
        </div>
        <h4 onClick={() => window.open(item.url, '_blank')}
          ...
          ${isCompleted ? 'text-slate-500 dark:text-slate-400' : 'text-slate-800 dark:text-slate-100 hover:text-brand-teal-600 dark:hover:text-brand-teal-400'}`}>
          {item.title}
        </h4>
        ...
        <div className="pt-3.5 border-t border-slate-100 dark:border-white/10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => window.open(item.url, '_blank')}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold font-outfit cursor-pointer transition-all border bg-brand-teal-50 border-brand-teal-100 text-brand-teal-600 hover:bg-brand-teal-100 dark:bg-brand-teal-500/15 dark:border-brand-teal-500/30 dark:text-brand-teal-300 dark:hover:bg-brand-teal-500/30">
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#060B18] transition-colors duration-300 font-outfit text-slate-900 dark:text-slate-50">
      ...
      <div aria-hidden className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-[20%] -left-[10%] w-[700px] h-[700px] rounded-full bg-brand-teal-500/5 dark:bg-brand-teal-500/10 blur-[80px]" />
        <div className="absolute top-[40%] -right-[15%] w-[600px] h-[600px] rounded-full bg-brand-blue-500/5 dark:bg-brand-blue-500/10 blur-[80px]" />
      ...
      >
        ...
        {/* ════════ HERO BANNER ════════ */}
        <div className="relative rounded-3xl overflow-hidden px-8 py-10 md:px-14 md:py-12 mb-12 border bg-gradient-to-br from-white via-brand-teal-50/50 to-brand-blue-50/50 border-brand-teal-100 shadow-sm dark:from-[#0f0a2e] dark:via-[#0E1F2B] dark:to-[#0d1a3a] dark:border-brand-teal-500/20 dark:shadow-none">
          <div className="hidden dark:block absolute -top-[40%] right-[5%] w-[400px] h-[400px] rounded-full bg-brand-blue-500/20 blur-[60px] z-0" />
          <div className="hidden dark:block absolute -bottom-[30%] left-[20%] w-[300px] h-[300px] rounded-full bg-brand-teal-500/20 blur-[50px] z-0" />
          ...
          <div className="relative z-10 flex items-center justify-between gap-8">
            <div className="max-w-[560px]">
              ...
              <h1 className="font-fraunces text-3xl md:text-5xl font-black leading-tight text-slate-900 dark:text-[#F1F5F9] mb-4 tracking-tight">
                ...
                <span className="bg-gradient-to-r from-brand-teal-600 to-brand-blue-600 dark:from-brand-teal-400 dark:to-brand-blue-400 bg-clip-text text-transparent">
                  Resources
                </span>
              </h1>
            ...
            <div className="hidden md:flex shrink-0 items-center justify-center w-[100px] h-[100px] rounded-full bg-brand-teal-50 border border-brand-teal-100 dark:bg-brand-teal-500/15 dark:border-brand-teal-500/25 backdrop-blur-md shadow-inner">
              <BrainCircuit className="w-12 h-12 text-brand-teal-500 dark:text-brand-teal-400" />
            </div>
          </div>
          ...
          <div className="relative z-10 flex flex-wrap gap-8 mt-10 pt-7 border-t border-slate-200 dark:border-white/10">
            {[
              ...
              <div key={i} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-brand-teal-100 border border-brand-teal-200 text-brand-teal-600 dark:bg-brand-teal-500/15 dark:border-brand-teal-500/20 dark:text-brand-teal-400">
                  {s.icon}
                </div>
                <div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 rounded-3xl text-center shadow-sm">
            <div className="w-20 h-20 rounded-full bg-brand-teal-50 dark:bg-slate-800 flex items-center justify-center mb-6">
              <BookOpen className="w-9 h-9 text-brand-teal-300 dark:text-brand-teal-500/40" />
            </div>
            ...
            <div className="flex items-center gap-2">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  className={`h-2 rounded-full transition-all duration-300 ${p === page ? 'w-7 bg-brand-teal-500 shadow-[0_0_8px_rgba(99,102,241,0.5)]' : 'w-2 bg-brand-teal-200 dark:bg-brand-teal-500/20'}`} />
              ))}
            </div>
