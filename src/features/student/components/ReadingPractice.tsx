...
const StatBadge = ({ label, value, color }: { label: string; value: string | number; color: string }) => (
  <div className="flex flex-col items-center bg-white rounded-2xl px-5 py-4 border border-brand-line shadow-sm min-w-[100px]">
    <span className={`font-manrope text-2xl font-black ${color}`}>{value}</span>
    <span className="font-jetbrains text-[10px] font-bold text-brand-text-mute uppercase tracking-[0.16em] mt-1">{label}</span>
  </div>
);
...
export default function ReadingPractice() {
  ...
  if (loadingPassage) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-brand-bg gap-4">
        <div className="w-16 h-16 rounded-2xl bg-brand-teal-wash flex items-center justify-center animate-pulse">
          <BookOpen className="w-8 h-8 text-brand-teal-700" />
        </div>
        <p className="text-brand-text-mute font-medium animate-pulse">Loading passage...</p>
      </div>
    );
  }
    ...
    <StudentLayout activeTab="reading" mainClassName="flex-1 p-4 sm:p-6 max-w-7xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {view === 'library' && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-brand-teal-400 to-brand-teal-600 rounded-2xl p-8 md:p-10 text-white shadow-lg relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div>
                <h1 className="font-manrope text-3xl font-extrabold mb-2 flex items-center gap-2">
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                    activeCategory === cat
                      ? 'bg-brand-teal-400 text-white shadow-md shadow-brand-teal-400/25'
                      : 'bg-white text-brand-text-mute border border-brand-line hover:border-brand-teal-400/50'
                  }`}
                >
                  {cat}
                </button>
          ...
          {loadingPassages ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {[1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} className="h-52 bg-brand-bg-alt rounded-2xl animate-pulse" />
              ))}
            </div>
          ) : filteredPassages.length === 0 ? (
            <Card className="border-none shadow-sm bg-white flex flex-col items-center justify-center p-16 text-center">
              <BookOpen className="h-12 w-12 text-brand-line mb-4" />
              <CardTitle className="font-manrope text-lg text-brand-text">No Passages Yet</CardTitle>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {filteredPassages.map(passage => (
                <Card
                  ...
                  className="group border border-brand-line shadow-sm bg-white hover:shadow-md hover:border-brand-teal-400 transition-all cursor-pointer flex flex-col h-52"
                >
                  <CardHeader className="pb-3 flex-none">
                    <div className="flex justify-between items-start gap-3">
                      <CardTitle className="font-manrope text-base font-bold text-brand-text group-hover:text-brand-teal-400 transition-colors line-clamp-2 leading-snug">
                        {passage.title}
                      </CardTitle>
                      <Badge className="bg-brand-teal-50 text-brand-teal-400 hover:bg-brand-teal-100 flex-shrink-0 text-xs">
                        {passage.category}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="flex flex-col flex-grow pt-0 pb-4">
                    <p className="text-brand-text-mute text-xs flex-grow line-clamp-2">
                      Source: {passage.source}
                    </p>
                    <div className="mt-auto pt-3 border-t border-brand-line flex justify-between items-center">
                      <span className="text-xs font-semibold text-brand-text-mute flex items-center gap-1">
                        <FileText className="w-3 h-3" /> {passage.wordCount} words
                      </span>
                      <span className="text-xs text-brand-teal-400 font-bold flex items-center gap-1 group-hover:translate-x-0.5 transition-transform">
      ...
      {view === 'reading' && selectedPassage && (
        <div className="flex flex-col h-full flex-1 min-h-[calc(100vh-140px)]">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
            <Button variant="ghost" onClick={handleRestart} className="text-brand-text-mute -ml-2 w-fit">
              <ArrowLeft className="w-4 h-4 mr-2" /> Back to Library
            </Button>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 bg-white border border-brand-line rounded-full px-4 py-2 shadow-sm">
                <Clock className="w-4 h-4 text-brand-teal-400" />
                <span className="font-mono font-bold text-brand-text text-sm">{formatTime(elapsed)}</span>
              </div>
              <Button onClick={handleSubmit} disabled={submitting} className="bg-brand-teal-400 hover:bg-brand-teal-500 text-white shadow-sm rounded-full">
                {submitting ? (
          ...
          <div className="flex flex-col lg:flex-row gap-5 flex-grow min-h-0">
            ...
            <div className="w-full lg:w-[55%] flex flex-col gap-4 overflow-y-auto pr-1 custom-scrollbar">
              <Card className="border-none shadow-sm bg-white flex-shrink-0">
                <CardHeader className="pb-4 border-b border-brand-line">
                  <div className="flex items-center gap-2 text-brand-teal-400 mb-2">
                    <BookOpen className="h-4 w-4" />
                    <span className="font-jetbrains text-[10.5px] font-bold uppercase tracking-[0.16em]">Reading Passage</span>
                  </div>
                  <CardTitle className="font-manrope text-xl font-bold text-brand-text leading-tight">
                    {selectedPassage.title}
                  </CardTitle>
                  <div className="flex items-center gap-3 mt-2">
                    <span className="text-xs text-brand-text-mute flex items-center gap-1">
                      <FileText className="w-3 h-3" /> {selectedPassage.wordCount} words
                    </span>
                    <Badge className="bg-brand-teal-50 text-brand-teal-400 text-xs">
                      {selectedPassage.category}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="text-brand-text text-[15px] leading-[1.9] whitespace-pre-line">
                    {selectedPassage.text}
                  </div>
                </CardContent>
              </Card>

              <Card className="border border-brand-teal-100 bg-brand-teal-50 flex-shrink-0 mb-4">
                <CardContent className="p-4 flex gap-3">
                  <AlertTriangle className="h-4 w-4 text-brand-teal-500 flex-shrink-0 mt-0.5" />
                  <ul className="text-xs text-brand-teal-700 space-y-1 list-disc list-inside">
            ...
            {selectedPassage.questions && selectedPassage.questions.length > 0 ? (
              <Card className="w-full lg:w-[45%] border border-brand-line shadow-sm bg-white flex flex-col overflow-hidden">
                <div className="p-4 border-b border-brand-line flex justify-between items-center bg-brand-bg-alt/50">
                  <h3 className="font-bold text-brand-text text-sm">Comprehension Questions</h3>
                  <Badge
                    variant="secondary"
                ...
                <div className="flex-grow overflow-y-auto p-5 space-y-7 custom-scrollbar">
                  {selectedPassage.questions.map((q, qi) => (
                    <div key={q.id} className="space-y-3">
                      <h4 className="text-sm font-semibold text-brand-text leading-relaxed">
                        <span className="text-brand-teal-400 font-black mr-1.5">Q{qi + 1}.</span>
                        {q.stem}
                      </h4>
                            ...
                            <div
                              ...
                              className={`relative p-3.5 rounded-xl border-2 cursor-pointer transition-all duration-200 flex items-center gap-3 ${
                                isSelected
                                  ...
                                  : 'border-brand-line bg-white hover:border-brand-teal-400/40'
                              }`}
                            >
                              <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center flex-shrink-0 text-xs font-black transition-all ${
                                isSelected
                                  ? 'border-brand-teal-400 bg-brand-teal-400 text-white'
                                  : 'border-brand-line text-brand-text-mute'
                              }`}>
                                {label}
                              </div>
                              <span className={`text-sm ${isSelected ? 'text-brand-teal-900 font-semibold' : 'text-brand-text'}`}>
                                {opt}
                              </span>
                            </div>
                          );
              ...
              <div className="w-full lg:w-[45%] flex flex-col items-center justify-center bg-white rounded-2xl border border-brand-line p-10 text-center gap-4">
                <BookOpen className="w-12 h-12 text-brand-line" />
                <p className="text-brand-text-mute text-sm">No comprehension questions for this passage. Submit to record your reading time and WPM.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {view === 'results' && results && selectedPassage && (
        <div className="space-y-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-gradient-to-br from-brand-teal-400 to-brand-teal-600 rounded-3xl p-8 text-white text-center relative overflow-hidden shadow-xl shadow-brand-teal-400/20">
            ...
            <div className="relative z-10">
              ...
              <p className="font-jetbrains text-brand-teal-100 text-[11px] font-bold uppercase tracking-[0.16em] mb-1">Your Grade</p>
              <div className={`font-manrope text-7xl font-black mb-3 ${results.grade === 'F' ? 'text-rose-300' : 'text-white'}`}>
                {results.grade}
              </div>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <StatBadge label="WPM" value={Math.round(results.wpm)} color="text-brand-teal-400" />
            ...
            <StatBadge label="Read Time" value={formatTime(results.readingTimeSeconds)} color="text-brand-text-mute" />
            <StatBadge label="Speed" value={results.speedCategory} color="text-brand-blue-500" />
          </div>

          {results.feedback && results.feedback.length > 1 && (
            <Card className="border-none shadow-sm bg-white">
              <CardHeader className="pb-3">
                <CardTitle className="font-manrope text-base font-bold flex items-center gap-2">
                  <Zap className="w-4 h-4 text-amber-500" /> Coach Insights
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {results.feedback.slice(1).map((tip, i) => (
                  <div key={i} className="flex gap-3 p-3 rounded-xl bg-brand-bg-alt">
                    <div className="w-1.5 h-1.5 rounded-full bg-brand-teal-400 mt-2 flex-shrink-0" />
                    <p className="text-sm text-brand-text">{tip}</p>
                  </div>
                ))}
              </CardContent>
            </Card>
          ...
          {results.scoredAnswers && results.scoredAnswers.length > 0 && (
            <Card className="border-none shadow-sm bg-white">
              <CardHeader className="pb-3">
                <CardTitle className="font-manrope text-base font-bold flex items-center gap-2">
                  <BarChart2 className="w-4 h-4 text-brand-teal-400" /> Answer Review
                </CardTitle>
              </CardHeader>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 space-y-2">
                        <p className="text-sm font-semibold text-brand-text">
                          <span className="text-brand-teal-400 mr-1">Q{i + 1}.</span>
                          {sa.stem}
                        </p>
                        <div className="space-y-1 text-xs">
                          <p className={`${sa.isCorrect ? 'text-emerald-700' : 'text-rose-700'}`}>
                          ...
                          {sa.explanation && (
                            <p className="text-brand-text-mute pt-1 border-t border-brand-line mt-1">
                              {sa.explanation}
                            </p>
                          )}
                        </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pb-8">
            ...
            <Button onClick={handleRestart} className="flex-1 bg-brand-teal-400 hover:bg-brand-teal-500 text-white">
