...
const SKILL_ACCENT: Record<string, { text: string; bg: string; border: string }> = {
  LISTENING: { text: "text-brand-teal-600",   bg: "bg-brand-teal-wash",   border: "border-brand-teal-200" },
...
const accent = (skill: string) =>
  SKILL_ACCENT[skill] ?? { text: "text-brand-text-mute", bg: "bg-brand-bg-alt", border: "border-brand-line" };
...
const CircleTimer: React.FC<{ timeLeft: number; total: number; size?: number }> = ({ timeLeft, total, size = 64 }) => {
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)", flexShrink: 0 }}>
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#D8E0E2" strokeWidth={6} />
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth={6}
        strokeDasharray={`${dash} ${circ}`} strokeLinecap="round"
        style={{ transition: "stroke-dasharray 0.5s linear, stroke 0.3s" }} />
      <text x={size/2} y={size/2+1} textAnchor="middle" dominantBaseline="middle"
        fill={isUrgent ? "#EF4444" : "#142B3A"} fontSize={size/4.2} fontWeight="900" fontFamily="monospace"
        ...
        <div className="flex items-center justify-between h-14 sm:h-16">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
              ...
              <span className="font-manrope text-base sm:text-xl font-semibold text-brand-text tracking-tight truncate">TestCrack</span>
            </div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <div className="flex items-center gap-1 sm:gap-2 bg-brand-teal-wash border border-brand-teal-200 px-2 sm:px-4 py-1 sm:py-1.5 rounded-full shadow-sm">
...
export default function FullMockAssessment() {
  ...
  const renderCriteriaRow = (met: boolean, label: string, detail?: string) => (
    <div className={`flex items-start gap-3 p-3 rounded-xl border ${met ? "bg-emerald-50 border-emerald-200" : "bg-brand-bg-alt border-brand-line"}`}>
      <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${met ? "bg-emerald-500" : "bg-brand-line"}`}>
        {met ? <CheckCircle2 className="w-3 h-3 text-white" /> : <XCircle className="w-3 h-3 text-white" />}
      </div>
      <div>
        <p className={`font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] ${met ? "text-emerald-700" : "text-brand-text-mute"}`}>{label}</p>
        {detail && <p className={`text-xs font-medium mt-0.5 ${met ? "text-emerald-600" : "text-brand-text-mute"}`}>{detail}</p>}
      </div>
    </div>
  );
  ...
  const renderNotEligible = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4">
        <div className="bg-white border border-brand-line rounded-2xl p-8 shadow-md">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 bg-brand-bg-alt border border-brand-line rounded-xl flex items-center justify-center"><Lock className="w-6 h-6 text-brand-text-mute" /></div>
            <div><p className="font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute">Not Yet Unlocked</p><h2 className="font-manrope text-2xl font-semibold text-brand-text tracking-tight">Mock IELTS</h2></div>
          </div>
          <p className="text-brand-text-mute font-medium text-sm mb-6">Complete all requirements below to unlock your Mock test.</p>
          ...
          <div className="bg-brand-bg-alt border border-brand-line rounded-xl p-4 mb-6">
            <p className="font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute mb-3">Skill Coverage</p>
            <div className="grid grid-cols-4 gap-2">
              {allSkills.map(skill => { const a = accent(skill); const covered = p.ia_per_skill[skill]; return (
                <div key={skill} className={`flex flex-col items-center gap-1.5 p-2.5 rounded-lg border ${covered ? `${a.bg} ${a.border}` : "bg-white border-brand-line"}`}>
                  <span className="text-xl">{SKILL_ICON[skill]}</span>
                  <span className={`text-[10px] font-semibold uppercase tracking-wider ${covered ? a.text : "text-brand-text-mute"}`}>{SKILL_LABEL[skill].slice(0, 3)}</span>
                  {covered ? <CheckCircle2 className={`w-3.5 h-3.5 ${a.text}`} /> : <div className="w-3.5 h-3.5 rounded-full border-2 border-brand-line" />}
                </div>
              ); })}
            </div>
          </div>
          <button onClick={() => navigate("/student/dashboard")} className="w-full py-3 border border-brand-line rounded-xl font-medium text-sm text-brand-text-mute hover:bg-brand-bg-alt transition-colors">Back to Dashboard</button>
        </div>
      </div>
    );
  };
  ...
  const renderMockAvailable = () => (
    <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4">
      <div className="bg-white border border-brand-line rounded-2xl p-8 sm:p-10 shadow-md">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-lg bg-brand-teal-600 text-white text-xs font-semibold tracking-wider uppercase mb-6 shadow-sm">
        ...
        <h1 className="font-manrope text-4xl font-bold text-brand-text tracking-tight mb-3">Ready for Your<br /><span className="text-brand-teal-600">Mock IELTS?</span></h1>
        <p className="text-brand-text-mute font-medium text-sm mb-6 leading-relaxed">
        ...
        <div className="grid grid-cols-4 gap-2 mb-8">
          {[
            ...
            <div key={s.skill} className={`${a.bg} border ${a.border} rounded-xl p-3 flex flex-col items-center gap-1.5`}>
              ...
              <span className="text-[10px] font-medium text-brand-text-mute">{s.time}</span>
            </div>
          ); })}
        </div>
        <div className="bg-brand-blue-50 border border-brand-blue-200 rounded-xl p-4 mb-6 flex items-start gap-3">
          <Clock className="w-5 h-5 text-brand-blue-600 flex-shrink-0 mt-0.5" />
          <div><p className="font-semibold text-brand-blue-800 text-sm">Individual section timers</p><p className="text-xs text-brand-blue-600 mt-0.5">Each section timer only starts when you begin that section. Once a section is locked, you cannot go back. Complete all sections within 24 hours.</p></div>
        </div>
        <div className="flex flex-col gap-3">
          <button onClick={() => void loadSession("STANDARD")} disabled={isLoading}
            className="w-full bg-brand-teal-700 hover:bg-brand-teal-600 disabled:opacity-60 text-white border-none font-semibold text-base uppercase tracking-wide py-4 rounded-xl flex items-center justify-center gap-2 shadow-sm hover:shadow-md transition-all">
            {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Loading…</> : <>Start Mock Test <ArrowRight className="w-5 h-5" /></>}
          </button>
          <button onClick={() => navigate("/student/dashboard")} className="w-full py-3 border border-brand-line rounded-xl font-medium text-sm text-brand-text-mute hover:bg-brand-bg-alt transition-colors">Cancel</button>
        </div>
        {mockStatus!.can_start_earned && (
          <div className="mt-6 border-t border-brand-line pt-6">
            <p className="font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute mb-3">Or Exchange Momentum</p>
            <button onClick={() => void loadSession("EARNED")} disabled={isLoading}
              className="w-full bg-brand-teal-700 hover:bg-brand-teal-600 disabled:opacity-60 text-white border-none rounded-xl py-3 font-semibold text-sm uppercase tracking-wide flex items-center justify-center gap-2 shadow-sm hover:shadow-md transition-all">
              <Zap className="w-4 h-4 text-amber-300 fill-amber-300" /> Extra Mock — {mockStatus!.earned_mock_cost.toLocaleString()} Momentum
            </button>
            <p className="text-[10px] text-brand-text-mute font-medium text-center mt-2">You have {mockStatus!.momentum_score.toLocaleString()} pts · Max 2 mocks / month</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4">
      <div className="bg-white border border-brand-line rounded-2xl p-8 shadow-md">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div><p className="font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute">This Month</p><h2 className="font-manrope text-2xl font-semibold text-brand-text tracking-tight">Session Expired</h2></div>
        </div>
        ...
        <div className="bg-brand-teal-wash border border-brand-teal-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          <Calendar className="w-5 h-5 text-brand-teal-600 flex-shrink-0" />
          <div><p className="font-semibold text-brand-teal-800 text-sm">Next standard slot opens</p><p className="text-brand-teal-600 font-medium text-sm">{firstOfNextMonth()}</p></div>
        </div>
        {mockStatus!.can_start_earned && (
          <div className="mb-4">
            <button onClick={() => void loadSession("EARNED")} disabled={isLoading}
              className="w-full bg-brand-teal-700 hover:bg-brand-teal-600 disabled:opacity-60 text-white border-none rounded-xl py-3 font-semibold text-sm uppercase tracking-wide flex items-center justify-center gap-2 shadow-sm transition-all">
    ...
    <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4">
      <div className="bg-white border border-brand-line rounded-2xl p-8 shadow-md">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div><p className="font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute">This Month</p><h2 className="font-manrope text-2xl font-semibold text-brand-text tracking-tight">Mock Completed</h2></div>
        </div>
        <div className="bg-brand-teal-wash border border-brand-teal-200 rounded-xl p-4 mb-6 flex items-center gap-3">
          <Calendar className="w-5 h-5 text-brand-teal-600 flex-shrink-0" />
          <div><p className="font-semibold text-brand-teal-800 text-sm">Next standard mock available</p><p className="text-brand-teal-600 font-medium text-sm">{firstOfNextMonth()}</p></div>
        </div>
        <p className="text-brand-text-mute text-sm font-medium mb-6">Your Real Band has been updated — keep drilling to build on it.</p>
        {mockStatus!.can_start_earned && (
          <div className="mb-4">
            <button onClick={() => void loadSession("EARNED")} disabled={isLoading}
              className="w-full bg-brand-teal-700 hover:bg-brand-teal-600 disabled:opacity-60 text-white border-none rounded-xl py-3 font-semibold text-sm uppercase tracking-wide flex items-center justify-center gap-2 shadow-sm transition-all">
    ...
    <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4">
      <div className="bg-white border border-brand-line rounded-2xl p-8 shadow-md">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div><p className="font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute">In Progress</p><h2 className="font-manrope text-2xl font-semibold text-brand-text tracking-tight">Mock Paused</h2></div>
        </div>
        <p className="text-brand-text-mute text-sm font-medium mb-6">You have an active mock session. Continue from your section dashboard.</p>
        <button onClick={() => void loadSession("STANDARD")} disabled={isLoading}
          className="w-full bg-brand-teal-700 hover:bg-brand-teal-600 disabled:opacity-60 text-white border-none font-semibold text-base uppercase tracking-wide py-4 rounded-xl flex items-center justify-center gap-2 shadow-sm hover:shadow-md transition-all">
          {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Loading…</> : <>Continue Mock <ArrowRight className="w-5 h-5" /></>}
        </button>
        <button onClick={() => navigate("/student/dashboard")} className="w-full py-3 mt-3 border border-brand-line rounded-xl font-medium text-sm text-brand-text-mute hover:bg-brand-bg-alt transition-colors">Back to Dashboard</button>
      </div>
    </div>
  );

  const renderGate = () => {
    if (statusLoading || !mockStatus) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="w-10 h-10 text-brand-teal-600 animate-spin" /></div>;
  ...
  const renderDashboard = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4 pb-12">
        {/* Header */}
        <div className="mb-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-brand-teal-600 text-white text-xs font-semibold tracking-wider uppercase mb-3 shadow-sm">
            <Trophy className="w-3.5 h-3.5" /> Mock IELTS
          </div>
          <h1 className="font-manrope text-2xl font-bold text-brand-text tracking-tight">Section Dashboard</h1>
          <p className="text-sm text-brand-text-mute mt-1">Start each section when ready. Once started, the section timer cannot be paused.</p>
        </div>

        {/* 24hr window countdown */}
        {windowRemainingMs > 0 && (
          <div className={`flex items-center gap-3 p-4 rounded-xl border mb-6 ${windowSec < 3600 ? "bg-rose-50 border-rose-200" : "bg-brand-bg-alt border-brand-line"}`}>
            <Clock className={`w-5 h-5 flex-shrink-0 ${windowSec < 3600 ? "text-rose-600" : "text-brand-text-mute"}`} />
            <div>
              <p className={`font-jetbrains text-[11px] font-semibold uppercase tracking-[0.14em] ${windowSec < 3600 ? "text-rose-700" : "text-brand-text-mute"}`}>24-hour window closes in</p>
              <p className={`text-xl font-bold font-mono ${windowSec < 3600 ? "text-rose-700" : "text-brand-text"}`}>{formatTime(windowSec)}</p>
            </div>
          </div>
        )}
          ...
          {MOCK_SKILL_ORDER.map(skill => {
            ...
            return (
              <div key={skill} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                isLocked     ? "bg-brand-bg-alt border-brand-line opacity-70" :
                isInProgress ? `${a.bg} ${a.border} shadow-sm` :
                               "bg-white border-brand-line hover:border-brand-teal-300 hover:shadow-sm"
              }`}>
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${isLocked ? "bg-brand-bg-alt" : a.bg}`}>
                    {SKILL_ICON[skill]}
                  </div>
                  <div>
                    <p className="font-semibold text-brand-text text-sm uppercase tracking-wide">{SKILL_LABEL[skill]}</p>
                    <p className="text-xs text-brand-text-mute font-medium">{SECTION_DURATION_MIN[skill]} min</p>
                ...
                <div>
                  {isLocked && (
                    <div className="flex items-center gap-1.5 text-xs font-bold text-brand-text-mute">
                  ...
                  {isInProgress && (
                    <button onClick={() => void resumeSection(skill)}
                      className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wide text-white bg-brand-teal-700 hover:bg-brand-teal-600 shadow-sm transition-colors`}>
      ...
      <div className="max-w-2xl mx-auto animate-fade-in pt-12 px-4">
        <div className="bg-white border border-brand-line rounded-2xl p-8 sm:p-10 shadow-md">
          <div className="flex items-center justify-between mb-6">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-lg bg-brand-teal-600 text-white text-xs font-semibold tracking-wider uppercase shadow-sm">
              <span className="text-base leading-none">🎧</span> Section 1 · Listening
            </div>
            {/* Live timer — visible so student knows time is running */}
            {sectionTimerSec >= 0 && (
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${isUrgent ? "bg-rose-50 border-rose-200" : "bg-brand-bg-alt border-brand-line"}`}>
                <CircleTimer timeLeft={sectionTimerSec} total={durationSec} size={36} />
                <div>
                  <p className={`text-[9px] font-semibold uppercase tracking-wider ${isUrgent ? "text-rose-600" : "text-brand-text-mute"}`}>Time left</p>
                  <p className={`text-sm font-bold font-mono leading-none ${isUrgent ? "text-rose-700" : "text-brand-text"}`}>{formatTime(sectionTimerSec)}</p>
                </div>
              </div>
            )}
          </div>
          <h1 className="font-manrope text-3xl sm:text-4xl font-bold text-brand-text tracking-tight mb-2">Before you begin</h1>
          <p className="text-brand-text-mute font-medium text-sm mb-8 leading-relaxed">Complete each set of questions as you listen. There's no separate transfer time.</p>
          ...
          <ul className="flex flex-col gap-2.5 mb-8">
            {["Use a quiet environment and headphones if possible.", "Your 30-minute section timer is already counting down.", "Answer as you listen — audio will not repeat."].map((tip, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm text-brand-text-mute font-medium">
                <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" /><span>{tip}</span>
              </li>
            ))}
          </ul>
          <button onClick={() => setPhase("session")}
            className="w-full bg-brand-teal-700 hover:bg-brand-teal-600 text-white border-none font-semibold text-base uppercase tracking-wide py-4 rounded-xl flex items-center justify-center gap-2 shadow-sm hover:shadow-md transition-all">
  ...
  const renderSession = () => {
    if (isLoading || !activeSection || !currentQ) {
      return (
        <div className="min-h-[70vh] flex flex-col items-center justify-center animate-fade-in">
          ...
          <p className="font-jetbrains text-brand-text-mute font-semibold uppercase tracking-[0.14em] text-sm">Loading Questions…</p>
        </div>
      );
    }
    ...
    return (
      <div className="max-w-6xl mx-auto pt-6 pb-16 px-4 animate-fade-in">
        ...
        {/* Section badge + timer */}
        <div className="flex items-center justify-between bg-white border border-brand-line rounded-2xl p-4 sm:p-6 mb-6 gap-4 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-brand-text font-semibold text-lg uppercase tracking-wide">{SKILL_LABEL[activeSection.skill] ?? activeSection.skill}</p>
              <p className="text-brand-text-mute text-xs font-medium uppercase tracking-wider mt-1">Question {currentIdx + 1} of {totalQ}</p>
            </div>
          </div>
          <div className="flex items-center self-end sm:self-auto bg-brand-bg-alt border border-brand-line px-4 py-2 rounded-xl">
            ...
            <div className="ml-3">
              <p className="text-[10px] font-semibold text-brand-text-mute uppercase tracking-wider">Section Timer</p>
              <p className="text-lg font-semibold text-brand-text leading-none">{formatTime(sectionTimerSec)}</p>
            </div>
          </div>
        </div>
            ...
            {activeSection.section_type === "AUDIO" && activeSection.audio_url ? (
              ...
              <div className="bg-white border border-brand-line rounded-2xl flex flex-col max-h-[700px] shadow-sm">
                ...
                <div className={`p-6 overflow-y-auto flex-1 ${!showPassage ? "hidden lg:block" : "block"}`}>
                  <p className="font-serif text-brand-text text-base leading-loose whitespace-pre-wrap">{activeSection.passage_text}</p>
                </div>
              </div>
            ) : (
              <div className={`${accent(activeSection.skill).bg} border ${accent(activeSection.skill).border} rounded-2xl p-6 hidden lg:flex flex-col items-center justify-center text-center gap-4`}>
                ...
                <p className="font-semibold text-brand-text uppercase tracking-wide">{SKILL_LABEL[activeSection.skill] ?? activeSection.skill}</p>
              </div>
            )}
          </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col gap-4">
            <div className="bg-white border border-brand-line rounded-2xl p-6 sm:p-8 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <span className="bg-brand-bg-alt text-brand-text-mute text-xs font-semibold uppercase tracking-wider px-4 py-1.5 rounded-lg border border-brand-line">Q {currentIdx+1} / {totalQ}</span>
                  ...
                  {optionKeys.map(key => {
                    ...
                    return (
                      <button key={key} onClick={() => setAnswers(p => ({ ...p, [currentQ.id]: key }))}
                        className={`text-left p-4 rounded-xl border font-medium text-sm transition-all flex items-start gap-3 ${selected ? "bg-brand-teal-600 border-brand-teal-600 text-white shadow-sm" : "bg-white border-brand-line text-brand-text hover:border-brand-teal-300 hover:bg-brand-bg-alt"}`}>
                  ...
                  {["TRUE", "FALSE", "NOT GIVEN"].map(val => {
                    ...
                    return (
                      <button key={val} onClick={() => setAnswers(p => ({ ...p, [currentQ.id]: val }))}
                        className={`p-4 rounded-xl border font-semibold text-sm uppercase tracking-wide transition-all ${selected ? `${color} border-transparent text-white shadow-sm` : "bg-white border-brand-line text-brand-text hover:border-brand-line hover:bg-brand-bg-alt"}`}>
                        {val}
                      </button>
                    );
                  })}
              ...
              {currentQ.question_type === "WRITING_PROMPT" && (
                <div>
                  ...
                  <div className="flex justify-between mt-2">
                    <p className="text-xs text-brand-text-mute font-medium">{(answers[currentQ.id] ?? "").trim().split(/\s+/).filter(Boolean).length} words</p>
                  ...
                  <div className="bg-brand-bg-alt border border-brand-line rounded-2xl p-6">
                    {isRecording ? (
                      <div className="flex flex-col items-center gap-4">
                        ...
                        {liveTranscript && <div className="w-full bg-white border border-brand-line rounded-xl p-3 text-sm text-brand-text font-medium italic min-h-[56px] max-h-[120px] overflow-y-auto">"{liveTranscript}"</div>}
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="bg-white border border-emerald-200 rounded-xl p-4 text-sm text-brand-text font-medium italic max-h-[120px] overflow-y-auto">"{answers[currentQ.id]}"</div>
                        <button onClick={() => startSpeakingRecording(currentQ.id)} className="text-sm font-semibold uppercase tracking-wide px-6 py-3 rounded-xl border border-brand-line bg-white text-brand-text hover:bg-brand-bg-alt self-start shadow-sm transition-colors">Re-record Answer</button>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-4 text-center">
                        ...
                        <p className="text-sm text-brand-text-mute font-medium max-w-xs">Tap the button and speak your answer. Your response will be transcribed automatically.</p>
                        <button onClick={() => startSpeakingRecording(currentQ.id)} className="bg-brand-teal-700 hover:bg-brand-teal-600 text-white border-none font-semibold text-sm uppercase tracking-wide px-8 py-4 rounded-xl shadow-sm hover:shadow-md transition-all">Start Speaking</button>
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="mt-8 flex gap-4">
                <button onClick={() => setCurrentIdx(i => i - 1)} disabled={currentIdx === 0}
                  className="px-6 py-4 border border-brand-line rounded-xl font-semibold text-brand-text-mute disabled:opacity-30 hover:bg-brand-bg-alt uppercase text-sm tracking-wide transition-colors">
                  Prev
                </button>
                <button onClick={handleNextQuestion}
                  disabled={!canProceed || (currentQ.question_type === "SPEAKING_PROMPT" && isRecording)}
                  className={`flex-1 font-semibold text-sm uppercase tracking-wide border-none rounded-xl py-4 transition-all ${!canProceed ? "bg-brand-bg-alt text-brand-text-mute opacity-60 cursor-not-allowed" : "bg-brand-teal-700 text-white hover:bg-brand-teal-600 shadow-sm hover:shadow-md"}`}>
  ...
  const renderScoring = () => (
    <div className="flex flex-col items-center justify-center min-h-[70vh] animate-fade-in">
      ...
      <h2 className="font-manrope text-3xl font-bold text-brand-text tracking-tight mb-3">Scoring Your Mock</h2>
      <p className="text-brand-text-mute font-medium text-lg">Calculating Real Band and updating your competency matrix.</p>
    </div>
  );
    ...
    return (
      <div className="max-w-3xl mx-auto animate-fade-in pt-8 pb-24 px-4">
        <div className="flex items-center justify-between mb-8">
          <h2 className="font-manrope text-3xl font-bold text-brand-text tracking-tight">Mock Complete</h2>
          <button onClick={() => { mockTimerStore.clearState(sessionIdRef.current ?? sessionId ?? ""); navigate("/student/dashboard", { state: { drillCompleted: true } }); }}
            className="px-6 py-3 bg-brand-teal-700 text-white border-none rounded-xl font-semibold text-sm uppercase hover:bg-brand-teal-600 shadow-sm hover:shadow-md transition-all">
            Dashboard
          </button>
        </div>

        <div className="bg-brand-ink rounded-2xl p-8 mb-6 text-center shadow-md relative overflow-hidden">
          <div className="absolute -top-8 -right-8 text-[140px] opacity-10 pointer-events-none select-none">🏆</div>
          <p className="font-jetbrains text-brand-mint text-[11px] font-semibold uppercase tracking-[0.16em] mb-1">Real Band Score</p>
          <div className="font-manrope text-8xl font-bold text-white leading-none mb-2">{mockResults?.real_band_score != null ? realBand.toFixed(1) : "—"}</div>
          {delta !== 0 && (
          ...
          {crossed && <div className="mt-3 inline-flex items-center gap-2 bg-amber-400 text-brand-text px-4 py-2 rounded-lg font-semibold text-sm uppercase shadow-sm">🎊 New Band Threshold Crossed!</div>}
          <p className="font-jetbrains text-brand-on-ink text-[10px] font-medium mt-3 uppercase tracking-[0.14em]">Real Band = Mock × 60% + Previous Matrix × 40%</p>
        </div>

        <div className="bg-white border border-brand-line rounded-2xl p-6 mb-6 text-center shadow-sm">
          <p className="font-jetbrains text-brand-text-mute text-[11px] font-semibold uppercase tracking-[0.14em] mb-1">Momentum Earned</p>
          <div className="font-manrope text-6xl font-bold text-brand-teal-600">+{momentum}</div>
          {breakdown.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {breakdown.map((b: any, i: number) => (
                <span key={i} className="bg-brand-teal-wash text-brand-teal-700 text-[10px] font-semibold uppercase tracking-wider px-3 py-1 rounded-full border border-brand-teal-200">+{b.points} {b.reason}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {skillScores.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
            {skillScores.map((s, i) => {
              ...
              return (
                <div key={i} className="bg-white border border-brand-line rounded-2xl p-6 shadow-sm flex flex-col">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <p className="font-semibold text-brand-text text-sm uppercase tracking-wide">{SKILL_LABEL[s.skill] ?? s.skill}</p>
                      <p className="text-brand-text-mute text-[10px] font-medium uppercase">{s.ai_graded ? "MCQ + AI Graded" : "Pure MCQ"}</p>
                    </div>
                  </div>
                  <p className="font-jetbrains text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute mb-0.5">Mock Score</p>
                  <p className="font-manrope text-5xl font-bold text-brand-text leading-none mb-1">{s.band > 0 ? s.band.toFixed(1) : "—"}</p>
                  {s.total > 0 && <p className="text-xs text-brand-text-mute font-medium mb-3">{s.correct} / {s.total} MCQ correct</p>}
                  <div className={`${a.bg} border ${a.border} rounded-xl p-3 mb-4`}>
                    <p className={`font-jetbrains text-[10px] font-semibold uppercase tracking-[0.14em] ${a.text} mb-1`}>Insight</p>
                    <p className="text-xs text-brand-text font-medium leading-relaxed">{insight}</p>
                  </div>
                  {subSkills.length > 0 && (
                    <div className="mb-4">
                      <p className="font-jetbrains text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute mb-2">Sub-skill Breakdown</p>
                      <div className="grid grid-cols-2 gap-1.5">
                        {subSkills.map((ss, j) => (
                          <div key={j} className="bg-brand-bg-alt border border-brand-line rounded-lg px-3 py-2">
                            <p className="text-[10px] font-semibold uppercase tracking-wider text-brand-text-mute">{SUBSKILL_LABEL[ss.sub_skill] ?? ss.sub_skill}</p>
                            <div className="flex items-baseline gap-1.5 mt-0.5">
                              <span className="text-lg font-bold text-brand-text">{ss.band.toFixed(1)}</span>
                              {ss.ai_band !== null && ss.ai_band !== undefined && <span className="text-[9px] text-brand-text-mute font-medium">AI: {ss.ai_band.toFixed(1)}</span>}
                            </div>
                            {ss.total_mcq > 0 && <p className="text-[9px] text-brand-text-mute font-medium mt-0.5">{ss.correct}/{ss.total_mcq} MCQ</p>}
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {hasAnyFeedback && (
                    <div className="mb-4">
                      <button onClick={() => toggleFeedback(i)} aria-expanded={isFeedbackOpen}
                        className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg bg-brand-bg-alt border border-brand-line hover:bg-brand-teal-wash transition-colors">
                        <span className="text-xs font-semibold uppercase tracking-wider text-brand-text-mute flex items-center gap-2">
                        ...
                        <ChevronDown className={`w-4 h-4 text-brand-text-mute transition-transform ${isFeedbackOpen ? "rotate-180" : ""}`} />
                      </button>
                      {isFeedbackOpen && (
                        <div className="mt-3 flex flex-col gap-3">
                          {feedbackItems.map((ss, j) => (
                            <div key={j} className="bg-white border border-brand-teal-200 rounded-xl overflow-hidden shadow-sm">
                              <div className="px-4 py-2.5 bg-brand-teal-wash border-b border-brand-teal-100 flex items-center gap-2">
                                <div className="w-1.5 h-4 bg-brand-teal-600 rounded-full" />
                                <p className="text-[10px] font-semibold uppercase tracking-wider text-brand-teal-700">{SUBSKILL_LABEL[ss.sub_skill] ?? ss.sub_skill}</p>
                              </div>
                              <div className="px-4 pt-3 pb-2">
                                <p className="text-[10px] font-semibold uppercase tracking-wider text-brand-text-mute mb-1">Summary</p>
                                <p className="text-xs text-brand-text-mute font-medium leading-relaxed italic">&ldquo;{ss.ai_feedback!.rationale}&rdquo;</p>
                              </div>
                              {(ss.ai_feedback!.key_observations?.length ?? 0) > 0 && (
                                <div className="px-4 pb-3">
                                  <p className="text-[10px] font-semibold uppercase tracking-wider text-brand-text-mute mt-1 mb-2">Key Observations</p>
                                  <ul className="flex flex-col gap-2">
                                    {ss.ai_feedback!.key_observations.map((obs, k) => (
                                      <li key={k} className="flex items-start gap-2"><span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-brand-teal-500 flex-shrink-0" /><span className="text-xs text-brand-text font-medium leading-relaxed">{obs}</span></li>
                                    ))}
                                  </ul>
                                </div>
                              )}
                  ...
                  <div className="border-t border-brand-line pt-3 mt-auto">
                    <p className="font-jetbrains text-[10px] font-semibold uppercase tracking-[0.14em] text-brand-text-mute mb-1.5">Real Band (Updated)</p>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-sm font-semibold text-brand-text-mute">{s.prev_matrix_band !== null ? s.prev_matrix_band.toFixed(1) : "—"}</span>
                      <span className="text-brand-text-mute text-xs">→</span>
                      <span className={`text-xl font-bold ${matrixUp ? "text-emerald-600" : "text-brand-text"}`}>{displayNewBand.toFixed(1)}</span>
                    ...
                    {s.diagnostic_band !== null && (
                      <div className={`flex items-center gap-2 text-xs font-medium ${diagUp ? "text-emerald-600" : "text-brand-text-mute"}`}>
        ...
        <p className="text-center text-brand-text-mute font-medium text-sm">Your Real Band and competency matrix have been updated. Keep drilling to improve your weakest skills before the next mock.</p>
      </div>
    );
  };
