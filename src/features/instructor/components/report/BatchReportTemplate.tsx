  ...
  return createPortal(
    ...
    >
      ...
      <div className="print:hidden sticky top-0 z-10 bg-white border-b border-slate-200 shadow-sm px-6 py-3 flex items-center justify-between">
        <span className="font-semibold text-slate-800 text-sm">Batch Report Preview</span>
      ...
      <div className="max-w-[800px] mx-auto bg-white shadow-lg my-8 p-12 print:shadow-none print:my-0 print:p-[1.5cm]">
        ...
        {/* Batch Info */}
        <div className="mb-8">
          <h1 className="text-2xl font-black text-slate-900 mb-1">{data.batch.name}</h1>
          <div className="text-sm text-slate-500 flex flex-wrap items-center gap-2">
            {data.batch.institute.name && (
              <span className="font-medium text-slate-700">{data.batch.institute.name}</span>
            )}
        ...
        {/* Engagement Today */}
        <div className="mb-8">
          <h2 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-3">Engagement Today</h2>
          <div className="bg-slate-50 rounded-xl border border-slate-200 px-6 py-4 flex items-center gap-10">
            {[
        ...
        <div>
          <h2 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-3">Assessment Performance</h2>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-slate-900">{data.ia_summary.avg_band.toFixed(1)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">Completion Rate</span>
                  <span className="font-bold text-slate-900">{data.ia_summary.completion_rate}%</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">High-Miss Count</span>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-slate-900">{data.mock_summary.avg_real_band.toFixed(1)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">At / Above Target</span>
                  <span className="font-bold text-emerald-700">{data.mock_summary.at_or_above_target}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-500">No Mock Yet</span>
      ...
      <div className="max-w-[800px] mx-auto bg-white shadow-lg my-8 p-12 print:shadow-none print:my-0 print:p-[1.5cm] print:break-before-page">
        <h2 className="text-lg font-black text-slate-900 mb-6">Student Performance Overview</h2>
            ...
            {sortedBandOverview.map((row, i) => {
              ...
              return (
                <tr key={row.user_id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                  <td className="px-3 py-2 pl-0 font-medium text-slate-800">{row.name}</td>
                  ...
                  <td className="px-3 py-2 text-slate-700">{ia?.ia_completed ?? '—'}</td>
                  <td className="px-3 py-2 text-rose-600">{ia?.ia_missed ?? '—'}</td>
                  <td className="px-3 py-2 text-slate-500">{row.last_ia_date ?? '—'}</td>
      ...
      <div className="max-w-[800px] mx-auto bg-white shadow-lg my-8 p-12 print:shadow-none print:my-0 print:p-[1.5cm] print:break-before-page">
        {/* At-Risk Students */}
        <div className="mb-10">
          <h2 className="text-lg font-black text-slate-900 mb-4">At-Risk Students</h2>
              ...
              <tbody>
                {data.at_risk.map((s, i) => (
                  <tr key={s.student_id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    <td className="px-3 py-2 pl-0 font-medium text-slate-800">{s.name}</td>
        ...
        {/* Diagnostic Status */}
        <div className="mb-10">
          <h2 className="text-lg font-black text-slate-900 mb-4">Diagnostic Status</h2>
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
            ...
            <tbody>
              {data.diagnostic_overview.map((d, i) => (
                <tr key={d.name + i} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                  <td className="px-3 py-2 pl-0 font-medium text-slate-800">{d.name}</td>
                  ...
                  <td className="px-3 py-2 pr-0">
                    {d.is_diagnosed ? (
                      ...
                      <span className="inline-flex px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold">
                        Pending
                      </span>
                    )}
                  </td>
